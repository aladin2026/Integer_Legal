...
public sealed class MatterParty : TenantEntity
{
    ...
    {
        RequireId(matterId, "Matter"); RequireId(partyId, "Party");
        MatterId = matterId; PartyId = partyId; RoleCode = Required(roleCode, 60, nameof(roleCode)).ToUpperInvariant(); IsAdverse = isAdverse;
    }
    public Guid MatterId { get; } public Guid PartyId { get; } public string RoleCode { get; } public bool IsAdverse { get; }
}
...
public sealed class Procedure : TenantEntity
{
    ...
    {
        RequireId(matterId, "Matter"); MatterId = matterId;
...
public sealed class Hearing : TenantEntity
{
    public Hearing(Guid id, Guid firmId, Guid procedureId, DateTimeOffset scheduledAt, string purpose, DateTimeOffset now) : base(id, firmId)
    {
        ...
        ProcedureId = procedureId; ScheduledAt = scheduledAt; Purpose = Required(purpose, 300, nameof(purpose));
    }
...
public sealed class LegalDocument : TenantEntity
{
    ...
    {
        RequireId(matterId, "Matter"); RequireId(createdBy, "Creator");
        MatterId = matterId; Title = Required(title, 300, nameof(title));
        Classification = Required(classification, 80, nameof(classification)); CreatedBy = createdBy;
    }
    public Guid MatterId { get; }
...
public sealed class DocumentVersion : TenantEntity
{
    ...
    {
        RequireId(documentId, "Document"); RequireId(createdBy, "Creator");
        ...
        DocumentId = documentId; VersionNumber = versionNumber; StorageObjectKey = Required(objectKey, 500, nameof(objectKey));
        ContentSha256 = normalizedHash; MediaType = Required(mediaType, 120, nameof(mediaType)); SizeBytes = sizeBytes; CreatedBy = createdBy;
    }
...
public sealed class TimeEntry : TenantEntity
{
    ...
    {
        RequireId(matterId, "Matter"); RequireId(userId, "User");
        if (minutes <= 0 || hourlyRate < 0) throw new DomainException("Time entry values are invalid.");
        MatterId = matterId; UserId = userId; ActivityDate = activityDate; Minutes = minutes;
        Description = Required(description, 500, nameof(description)); Billable = billable; HourlyRate = hourlyRate; Currency = CurrencyCode(currency);
    }
    public Guid MatterId { get; } public Guid UserId { get; } public DateOnly ActivityDate { get; }
    public int Minutes { get; } public string Description { get; } public bool Billable { get; }
    public decimal? HourlyRate { get; } public string Currency { get; }
}
...
public sealed class ExpenseEntry : TenantEntity
{
    ...
    {
        RequireId(matterId, "Matter"); if (amount < 0) throw new DomainException("Expense amount cannot be negative.");
        MatterId = matterId; IncurredOn = incurredOn; Description = Required(description, 500, nameof(description));
        Amount = amount; Currency = CurrencyCode(currency); Billable = billable;
    }
    public Guid MatterId { get; } public DateOnly IncurredOn { get; } public string Description { get; }
    public decimal Amount { get; } public string Currency { get; } public bool Billable { get; }
}
...
public sealed class Budget : TenantEntity
{
    public Budget(Guid id, Guid firmId, Guid matterId, decimal amount, string currency, decimal warningPercent) : base(id, firmId)
    {
        ...
        MatterId = matterId; Amount = amount; Currency = CurrencyCode(currency); WarningPercent = warningPercent;
    }
    public Guid MatterId { get; } public decimal Amount { get; } public string Currency { get; } public decimal WarningPercent { get; }
}
...
public sealed class Prebill : TenantEntity
{
    ...
    {
        RequireId(matterId, "Matter"); RequireId(createdBy, "Creator");
        MatterId = matterId; Reference = Required(reference, 60, nameof(reference)).ToUpperInvariant();
        Currency = CurrencyCode(currency); CreatedBy = createdBy; Status = PrebillStatus.Draft;
    }
    public Guid MatterId { get; } public string Reference { get; } public string Currency { get; }
    public Guid CreatedBy { get; } public PrebillStatus Status { get; }
}
...
public sealed class PrebillLine : TenantEntity
{
    public PrebillLine(Guid id, Guid firmId, Guid prebillId, short lineType, Guid? sourceId, string description, decimal quantity, decimal unitPrice) : base(id, firmId)
    {
        ...
        PrebillId = prebillId; LineType = lineType; SourceId = sourceId; Description = Required(description, 500, nameof(description));
        Quantity = quantity; UnitPrice = unitPrice;
    }
    public Guid PrebillId { get; } public short LineType { get; } public Guid? SourceId { get; }
    public string Description { get; } public decimal Quantity { get; } public decimal UnitPrice { get; }
}
...
public sealed class RoleAssignment : TenantEntity
{
    ...
    {
        RequireId(userId, "User"); RequireId(grantedBy, "Grantor");
        if (validUntil <= validFrom) throw new DomainException("Role validity interval is invalid.");
        UserId = userId; RoleCode = Required(roleCode, 80, nameof(roleCode)).ToUpperInvariant(); MatterId = matterId;
        GrantedBy = grantedBy; ValidFrom = validFrom; ValidUntil = validUntil;
    }
    public Guid UserId { get; } public string RoleCode { get; } public Guid? MatterId { get; }
    public Guid GrantedBy { get; } public DateTimeOffset ValidFrom { get; } public DateTimeOffset? ValidUntil { get; }
}
...
public sealed class ScheduledCommand : TenantEntity
{
    public ScheduledCommand(Guid id, Guid firmId, string commandType, int version, string payload, DateTimeOffset executeAt) : base(id, firmId)
    {
        ...
        CommandType = Required(commandType, 160, nameof(commandType)); Version = version;
        Payload = Required(payload, 100_000, nameof(payload));
        try { using var _ = JsonDocument.Parse(Payload); }
        catch (JsonException exception) { throw new DomainException("Scheduled command payload must be valid JSON.", exception); }
        ExecuteAt = executeAt;
    }
    public string CommandType { get; } public int Version { get; } public string Payload { get; } public DateTimeOffset ExecuteAt { get; }
}
...
internal static class Guard
{
    public static string Required(string? value, int max, string name)
    {
        ...
        if (normalized.Length is 0 || normalized.Length > max) throw new DomainException($"{name} is invalid.");
        return normalized;
    }
    ...
    public static void RequireId(Guid value, string name) { if (value == Guid.Empty) throw new DomainException($"{name} identifier is required."); }
